...
public class DashboardService {
  ...
  public DashboardChartDTO getAdminDashboardChart(LocalDate start, LocalDate end) {
    List<AdminEvent> logs =
        adminEventRepository.findByCreatedAtBetween(start.atStartOfDay(), end.atTime(23, 59, 59));
  ...
  public DashboardChartDTO getAdminDashboardChartById(Long id, LocalDate start, LocalDate end) {
    List<AdminEvent> logs =
        adminEventRepository.findByCreatedAtBetweenAndAdminId(
            start.atStartOfDay(), end.atTime(23, 59, 59), id);
  ...
  public DashboardChartDTO getUserDashboardChart(LocalDate start, LocalDate end) {
    List<UserEvent> logs =
        userEventRepository.findByCreatedAtBetweenAndUserIdNotNull(
  ...
  public DashboardChartDTO getUserDashboardChartById(Long id, LocalDate start, LocalDate end) {
    List<UserEvent> logs =
        userEventRepository.findByCreatedAtBetweenAndUserId(
            start.atStartOfDay(), end.atTime(23, 59, 59), id);
  ...
  public DashboardChartDTO getGuestDashboardChart(LocalDate start, LocalDate end) {
    List<UserEvent> logs =
        userEventRepository.findByCreatedAtBetweenAndUserIdNull(
            start.atStartOfDay(), end.atTime(23, 59, 59));
    ...
    return logs.stream()
        .collect(
            Collectors.groupingBy(
                e -> e.getEventType().name(), Collectors.summingLong(BaseEvent::getQuantity)));
  }
            ...
            Collectors.toMap(
                ...
                e ->
                    e.getValue().entrySet().stream()
                        .map(d -> new ChartPoint(d.getKey(), d.getValue()))
  ...
  private <T extends BaseEvent> TokenSummary buildAiTokenSummary(List<T> aiEvents) {
    return aiEvents.stream()
        .map(e -> parseAiMetadata(e.getMetadata()))
        .reduce(
            new TokenSummary(),
            (a, b) -> {
    ...
    return aiEvents.stream()
        ...
        .collect(
            Collectors.toMap(
                ...
                entry -> {
                  long totalIn =
                      entry.getValue().stream().mapToLong(TokenSummary::getInputToken).sum();
                  long totalOut =
                      entry.getValue().stream().mapToLong(TokenSummary::getOutputToken).sum();
