...
public class DictionaryEntryService {
  ...
  public List<DictionaryEntry> createWordByGemini(Long currentAdminId, String word)
      throws Exception {
    ...
    dictionaryEntries =
        Arrays.stream(dtoArray).map(this::toEntity).map(dictionaryEntryRepository::save).toList();
  ...
  public Page<DictionaryEntry> findAllPaged(
  ...
  public DictionaryEntry createWord(DictionaryEntryDTO dto) {
    ...
    if (existed) {
      throw new BusinessException(
          409, "The word '" + dto.getHeadword() + "' already exists in the system.");
    }
