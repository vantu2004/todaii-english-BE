...
public class SecurityConfigForAdmin {
  ...
  private void configureCors(org.springframework.security.config.annotation.web.configurers.CorsConfigurer<HttpSecurity> cors) {
    cors.configurationSource(request -> {
      ...
      String origin = StringUtils.isBlank(allowedOrigin) ? "http://localhost:5173" : allowedOrigin;
  ...
  private void configureAuthorizations(AuthorizeHttpRequestsConfigurer<HttpSecurity>.AuthorizationManagerRequestMatcherRegistry auth) {
    auth
            ...
            // 2. Dashboard
            .requestMatchers("/api/v1/dashboard/**").hasAnyAuthority(AdminAuthorities.ALL_ADMINS)

            // 3. Quản lý Admin & Settings (Hầu hết là Super Admin)
            .requestMatchers(HttpMethod.GET, "/api/v1/admin/me").hasAnyAuthority(AdminAuthorities.ALL_ADMINS)
            .requestMatchers(HttpMethod.PUT, "/api/v1/admin/me").hasAnyAuthority(AdminAuthorities.ALL_ADMINS)
            .requestMatchers("/api/v1/admin/**", "/api/v1/setting/**").hasAuthority(AdminAuthorities.SUPER)

            // 4. Quản lý User
            .requestMatchers("/api/v1/user/**").hasAnyAuthority(AdminAuthorities.SUPER_AND_USER)
...
//                        .hasAnyAuthority("SUPER_ADMIN", "CONTENT_MANAGER")
//                        .requestMatchers(HttpMethod.POST, "/api/v1/toeic/test/*/part-12/*/question")
//                        .hasAnyAuthority("SUPER_ADMIN", "CONTENT_MANAGER")
//                        .requestMatchers(HttpMethod.PUT, "/api/v1/toeic/part-12/question/*")
//                        .hasAnyAuthority("SUPER_ADMIN", "CONTENT_MANAGER")
//                        .requestMatchers(HttpMethod.POST, "/api/v1/toeic/test/*/part-34567/*/question")
//                        .hasAnyAuthority("SUPER_ADMIN", "CONTENT_MANAGER")
