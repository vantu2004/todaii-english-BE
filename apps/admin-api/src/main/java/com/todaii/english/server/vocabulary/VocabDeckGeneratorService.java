...
public class VocabDeckGeneratorService {
  ...
  public VocabDeck autoGenerateDeckWords(Long currentAdminId, Long deckId) {
    VocabDeck deck =
        vocabDeckRepository
            .findById(deckId)
    ...
    Set<String> wordSet =
        createSuggestedWords(currentAdminId, deck).stream()
            .map(String::trim)
            .map(String::toLowerCase)
            .collect(Collectors.toSet());

    // Lấy các từ đã có trong DB (dựa theo headword)
    List<DictionaryEntry> existingEntries =
        dictionaryEntryRepository.findByHeadwordInIgnoreCase(wordSet);
    ...
    Set<String> existingHeadwords =
        existingEntries.stream()
            .map(e -> e.getHeadword().toLowerCase())
            .collect(Collectors.toSet());
    Set<String> missingWords =
        wordSet.stream()
            .filter(word -> !existingHeadwords.contains(word))
            .collect(Collectors.toSet());
    if (missingWords.isEmpty()) {
      deck.getWords().addAll(finalWords);
      return vocabDeckRepository.save(deck);
    }

    // Lấy dữ liệu thô từ Dictionary API và refine bằng Gemini
    handleMissingWords(currentAdminId, missingWords, finalWords);
  ...
  private Set<String> createSuggestedWords(Long currentAdminId, VocabDeck deck) {
    // Lấy các headword hiện có trong deck (nếu có)
    Set<String> existingWords =
        Optional.ofNullable(deck.getWords()).orElse(Collections.emptySet()).stream()
            .map(e -> e.getHeadword().toLowerCase())
            .collect(Collectors.toSet());

    String prompt = buildDeckPrompt(deck, existingWords);
    ...
    String rawResponse = aiResponse.getText();

    return parseWordSet(rawResponse);
  }

  // Xây dựng prompt để Gemini sinh từ vựng phù hợp
  private String buildDeckPrompt(VocabDeck deck, Set<String> existingWords) {
    String avoidList = existingWords.isEmpty() ? "none" : String.join(", ", existingWords);

    return String.format(
        Gemini.DECK_WORDS_PROMPT,
        deck.getCefrLevel(),
        deck.getName(),
        deck.getDescription(),
        avoidList);
  ...
  private void handleMissingWords(
      Long currentAdminId, Set<String> missingWords, Set<DictionaryEntry> finalWords) {
    try {
      ...
      for (String word : missingWords) {
        try {
          DictionaryApiResponse[] response =
              dictionaryEntryService.lookupWord(currentAdminId, word);
      ...
      String refinePrompt =
          String.format(Gemini.DICTIONARY_PROMPT, rawJson, String.join(", ", missingWords));

      // Gọi Gemini refine dữ liệu
      AIResponse aiResponse = geminiPort.generateText(refinePrompt);
      eventService.logAdmin(
          currentAdminId,
          EventType.AI_REQUEST,
          1,
      ...
      String refinedResponse = aiResponse.getText();
  ...
  private Set<DictionaryEntry> parseDictionaryBatch(String rawResponse) {
    try {
      DictionaryEntryDTO[] dtoArray =
          objectMapper.readValue(rawResponse, DictionaryEntryDTO[].class);
      return Arrays.stream(dtoArray)
          .map(dictionaryEntryService::toEntity)
          .collect(Collectors.toSet());
    } catch (Exception e) {
