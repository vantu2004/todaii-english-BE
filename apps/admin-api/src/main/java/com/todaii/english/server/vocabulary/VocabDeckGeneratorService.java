...
public class VocabDeckGeneratorService {
	...
	public VocabDeck autoGenerateDeckWords(Long currentAdminId, Long deckId) {
		...
		Set<String> wordSet = createSuggestedWords(currentAdminId, deck).stream().map(String::trim)
	...
	private Set<String> createSuggestedWords(Long currentAdminId, VocabDeck deck) {
		...
		Set<String> existingWords = Optional.ofNullable(deck.getWords()).orElse(Collections.emptySet()).stream()
				.map(e -> e.getHeadword().toLowerCase()).collect(Collectors.toSet());
	...
	private String buildDeckPrompt(VocabDeck deck, Set<String> existingWords) {
		...
		return String.format(Gemini.DECK_WORDS_PROMPT, deck.getCefrLevel(), deck.getName(), deck.getDescription(),
				avoidList);
	}
	...
	private void handleMissingWords(Long currentAdminId, Set<String> missingWords, Set<DictionaryEntry> finalWords) {
		try {
			List<DictionaryApiResponse[]> rawApiResponses = new ArrayList<>();

			for (String word : missingWords) {
				try {
					DictionaryApiResponse[] response = dictionaryEntryService.lookupWord(currentAdminId, word);
