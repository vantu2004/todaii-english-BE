...
public class ArticleSpecification {
  ...
  public static Specification<Article> hasKeyword(String keyword) {
    return (root, query, cb) -> {
      ...
      return cb.or(
          cb.like(cb.lower(root.get(Article_.SOURCE_ID)), likeValue),
  ...
  public static Specification<Article> hasSourceName(String sourceName) {
    return (root, query, cb) ->
        sourceName == null
            ? null
            : cb.like(
                cb.lower(root.get(Article_.SOURCE_NAME)), "%" + sourceName.toLowerCase() + "%");
  }

  public static Specification<Article> hasCefrLevel(CefrLevel level) {
    return (root, query, cb) ->
        level == null ? null : cb.equal(root.get(Article_.CEFR_LEVEL), level);
  }

  public static Specification<Article> hasMinViews(Integer minViews) {
    return (root, query, cb) ->
        minViews == null ? null : cb.greaterThanOrEqualTo(root.get(Article_.VIEWS), minViews);
  }
