...
public class ToeicTestSessionService {
  ...
  //      session.setTimeSpent(request.getTimeSpent());
  //    } else {
  //      long seconds = Duration.between(session.getStartedAt(),
  // session.getCompletedAt()).toSeconds();
  ...
  //
  //    // Ensure there is a UserAnswer record for every question in the session (non-answered
  // questions
