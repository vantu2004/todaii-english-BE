...
public class VideoService {
  ...
  public List<Video> getRelatedVideos(Long videoId, int limit) {
    ...
    if (related.isEmpty()) {
      return videoRepository.findFallbackByCefr(
          videoId, video.getCefrLevel(), PageRequest.of(0, limit));
    }

    if (related.size() < limit) {
      int remain = limit - related.size();
      List<Video> fallback =
          videoRepository.findFallbackByCefr(
              videoId, video.getCefrLevel(), PageRequest.of(0, remain));
  ...
  public Page<Video> filterVideos(
    ...
    Specification<Video> spec =
        VideoSpecification.isEnabled()
            .and(VideoSpecification.hasKeyword(keyword))
            .and(VideoSpecification.hasCefrLevel(cefrLevel))
            .and(VideoSpecification.hasMinViews(minViews))
  ...
  public Page<Video> getVideosByDate(
      LocalDate parsedDate, int page, int size, String sortBy, String direction) {
    Sort sort = Sort.by(Sort.Direction.fromString(direction), sortBy);
    Pageable pageable = PageRequest.of(page - 1, size, sort);
