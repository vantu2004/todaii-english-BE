...
public class VideoSpecification {
  ...
  public static Specification<Video> hasKeyword(String keyword) {
    return (root, query, cb) -> {
      ...
      return cb.or(
          cb.like(cb.lower(root.get(Video_.title)), likeValue),
  ...
  public static Specification<Video> hasCefrLevel(CefrLevel cefrLevel) {
    return (root, query, cb) ->
        cefrLevel == null ? null : cb.equal(root.get(Video_.cefrLevel), cefrLevel);
  }

  public static Specification<Video> hasMinViews(Integer minViews) {
    return (root, query, cb) ->
        minViews == null ? null : cb.greaterThanOrEqualTo(root.get(Video_.VIEWS), minViews);
  }
