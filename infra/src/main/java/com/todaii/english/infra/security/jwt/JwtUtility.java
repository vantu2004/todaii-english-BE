...
public class JwtUtility {
  ...
  public String generateAccessToken(JwtPrincipal principal) {
    ...
    Date now = new Date();
    Date expiry =
        new Date(now.getTime() + SecurityConstants.ACCESS_TOKEN_EXPIRATION_MINUTES * 60 * 1000L);

    return Jwts.builder()
        .issuer(jwtProperties.getIssuer())
        .subject(subject)
        .claim("roles", String.join(",", principal.getRoleCodes())) // User thì rỗng
        .claim("actorType", principal.getActorType())
        .issuedAt(now)
        .expiration(expiry)
        .signWith(Keys.hmacShaKeyFor(jwtProperties.getSecret().getBytes()), Jwts.SIG.HS512)
        .compact();
  }
  ...
  public Claims validateAccessToken(String token) throws JwtValidationException {
    try {
      SecretKeySpec secretKeySpec =
          new SecretKeySpec(
              jwtProperties.getSecret().getBytes(), SecurityConstants.SECRET_KEY_ALGORITHM);
